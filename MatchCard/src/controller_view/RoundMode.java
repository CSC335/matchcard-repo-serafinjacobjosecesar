...
/**
 * RoundMode public class that extends Border Pane represents a round based game
 * mode
 */
public class RoundMode extends BorderPane {
	 ...
	 * @param currDeck : AbstractCardCollection that represents the current card
	 *                 collection in play
	 */
...
//		deck = new AnimalCollection(col, player.getCurrCardBack());
	...
	public void initClock() {
		...
		timerliner = new Timeline(new KeyFrame(Duration.seconds(1), event -> {
			clock++;
	...
	/**
	 * updateGame Updates the game, adds another column and row to the board. Adds
	 * more cards to the deck
	 */
